...
public class ReservasView extends JFrame {
	...
	public static JComboBox<FormasDePago> txtFormaPago;
	int xMouse, yMouse;
	...
	public ReservasView() {
		...
		// se hace una nueva instancia de ReservasController
		this.reservaController = new ReservasController();
		this.formaDePagoController = new FormaDePagoController();
		...
		// Se altero el ancho porque se veia mal
...
//		txtFormaPago.setModel(new DefaultComboBoxModel(new String[] {"Tarjeta de Crédito", "Tarjeta de Débito", "Dinero en efectivo"}));
		var formas = this.formaDePagoController.listaFormasDePago();
		txtFormaPago.addItem(new FormasDePago(0, "Seleccione método de pago"));
		formas.forEach(forma -> txtFormaPago.addItem(forma));
	...
	private void guardarReserva() {
		...
		try {
			// nota
			if (txtFechaEntrada.getDate() != null && txtFechaSalida.getDate() != null && txtValor.getText() != " "
					&& txtFormaPago.getSelectedIndex() >0) {

				// seguir investigando
				LocalDate dataE = LocalDate
						.parse(((JTextField) txtFechaEntrada.getDateEditor().getUiComponent()).getText());
				LocalDate dataS = LocalDate
						.parse(((JTextField) txtFechaSalida.getDateEditor().getUiComponent()).getText());
				BigDecimal valor = new BigDecimal(txtValor.getText());
				var formaPago = (FormasDePago) txtFormaPago.getSelectedItem();


				int respuesta = JOptionPane.showConfirmDialog(null, "Registrar Usuario");

				if (respuesta == JOptionPane.YES_OPTION) {
			Reservas reserva =new Reservas(dataE, dataS, valor) ; 
			reservaController.guardarReserva(reserva, formaPago.getId_formaPago());
					RegistroHuesped registro = new RegistroHuesped(reserva.getIdReserva()) ;
					registro.setVisible(true);
					dispose();
				} else {
					JOptionPane.showMessageDialog(this, "No se registro la reserva");
				}
			}
		else {
	...
	public void calcularValor(JDateChooser dateE, JDateChooser dateS) {
		if (dateE.getDate() != null && dateS.getDate() != null) {
			if (dateE.getDate().after(dateS.getDate())) {
				...
				txtValor.setText("Seleccione método de pago");
				return;
			}
